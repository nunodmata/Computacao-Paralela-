//changed - now it displays the execution time correctly even when changing the number of threads
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
#include<string.h>
...
#include <cmath>
#include <omp.h>
#include <xmmintrin.h> //  para usar alignas
#include <chrono>
#include <iostream>
...
// alignas(32) for memory alignment instructing the compiler to align the allocated memory for the variable or type to a 32-byte boundary.
//  Position
alignas(32) double r[MAXPART][3]; //added alignas
//  Velocity
alignas(32) double v[MAXPART][3];
//  Acceleration
alignas(32) double a[MAXPART][3];
//  Force
alignas(32) double F[MAXPART][3];
...
int main()
{
    ...
    //strcpy(prefix, "test");
    scanf("%s",prefix); //Mudar se quisermos vltar a inserir argumento
    ...
    scanf("%s",atype);  //Mudar se quisermos vltar a inserir argumento
...
//    printf("Execution time: %f seconds\n", execution_time);
    ...
    // Calcular o tempo de execuï¿½ï¿½o
    std::chrono::duration<double> elapsed = end - start;

    // Exibir o tempo de execuï¿½ï¿½o
...
#pragma omp parallel for collapse(3) // treat the three loops as a single long loop. one iteration
    for (int x = 0; x < n; x++) {
        for (int y = 0; y < n; y++) {
            for (int z = 0; z < n; z++) {
                int p = x * n * n + y * n + z; // calculates the linear index p from x, y, z
                if (p < N) {
                    r[p][0] = (x + 0.5) * pos;
                    r[p][1] = (y + 0.5) * pos;
                    r[p][2] = (z + 0.5) * pos;
...
double MeanSquaredVelocity() { 
    ...
    // Loop iterations are made more cache-friendly (cache locallity) -
	// it lights up continuous elements in memory
    for (int i = 0; i < 3; i++) {
...
//allows this calculation to be done in parallel way with each thread contributing to the total sum of kinetic energy       
...
double Potential() {
    ...
    // thread-local storage to avoid bad memory sharing / data races
    #pragma omp parallel
    {
        double localPot = 0.0;
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                if (j != i) {
                    double diff[3] = {r[i][0] - r[j][0], r[i][1] - r[j][1], r[i][2] - r[j][2]};
                    double r2 = diff[0] * diff[0] + diff[1] * diff[1] + diff[2] * diff[2];
                    double rnorm = __builtin_sqrt(r2); //  __builtin_sqrt é mais beneficial 
                    double quot = sigma / rnorm;
                    double quot12 = quot * quot * quot * quot * quot * quot;
                    quot12 = quot12 * quot12; // quot to the power of 12
                    double quot6 = quot12 / (quot * quot * quot * quot * quot * quot); // quot to the power of 6

                    localPot += 4 * epsilon * (quot12 - quot6);
                }
            }
        }
...
void computeAccelerations() {
    // Zeroes the acceleration of all particles
    ...
    // Variables to calculate the force between particles
    double rij[3], f[3], rSqd;

    // Compute accelerations between pairs of particles
    #pragma omp parallel for private(rij, f, rSqd)
    for (int i = 0; i < N - 1; i++) {
        for (int j = i + 1; j < N; j++) {
            rSqd = 0;
            ...
            // Calculate the force factor according to the Lennard-Jones potential
            double forceFactor = 24 * (2 * pow(rSqd, -7) - pow(rSqd, -4));

            // Calcula as componentes da forÃ§a
            for (int k = 0; k < 3; k++) {
                f[k] = forceFactor * rij[k];
            }

            // Update the accelerations of i and j atomically to avoid "data races"
            for (int k = 0; k < 3; k++) {
                #pragma omp atomic
                a[i][k] += f[k];
                #pragma omp atomic
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    // Update positions and velocities with parallelism
    #pragma omp parallel for
    for (i = 0; i < N; i++) {
        for (j = 0; j < 3; j++) {
    ...
    // Final speed update with parallelism
    #pragma omp parallel for
    for (i = 0; i < N; i++) {
        for (j = 0; j < 3; j++) {
            v[i][j] += 0.5 * a[i][j] * dt;
    ...
    // Each thread accumulates its own partial sum of pressure. At the end of the loop, it combines all these partial sums into "psum"
