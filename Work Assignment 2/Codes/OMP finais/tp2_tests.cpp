//changed OMP TP2 faz display do tempo de execucao corretamente mesmo mudando o nr de threads 
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
#include<string.h>
...
double Tinit;  //voltar a mudar se quisermos passar como argumento
...
#pragma omp parallel for private(x,y,z)
	for (int i = 0; i < n * n * n; i++) {
	    ...
	    int y = (i / n) % n;
	    int z = i / (n * n);
	// Use uma diretiva critical para proteger o acesso à variável compartilhada p
    #pragma omp critical
	    if (p < N) {
	        r[p][0] = (x + 0.5) * pos;
	        r[p][1] = (y + 0.5) * pos;
...
double MeanSquaredVelocity() { 
	...
	// porque acede a elementos continuos na memoria (para tentar tornar esta funcao mais escalavel com o valor de N)
    for (int i = 0; i < 3; i++) {
...
double Potential() {
    ...
    int j;
    memset(precalculated, 0, N * sizeof(double));

#pragma omp parallel for reduction(+:Pot) private(j, r2, rnorm, term1, term2)
    for (int i = 0; i < N; i++) {
        double localPot = 0.0;
        for (int j = 0; j < N; j++) {
            if (j != i) {
                ...
                rnorm = __builtin_sqrt(r2);  //__builtin_sqrt(r2); //sqrt(r2);  // usamos uma função intrínseca para melhorar SIMD
                quot = sigma / rnorm;
                term1 = quot * quot * quot * quot * quot * quot * quot * quot * quot * quot * quot * quot;
                term2 = quot * quot * quot * quot * quot * quot;
                localPot += 4 * epsilon * (term1 - term2);
            }
        }
        Pot += localPot;
    }
...
#pragma omp parallel for private(j, k, rSqd, rij, f)
	for (int i = 0; i < N - 1; i++) {
		double localA[3] = {0.0, 0.0, 0.0};
    	for (j = 0; j < N; j++) {
	        if (i != j) {
			    double rSqd = 0;
			    for (int k = 0; k < 3; k++) {
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    // Atualização de posições e velocidades com paralelismo
    #pragma omp parallel for
    for (i = 0; i < N; i++) {
        for (j = 0; j < 3; j++) {
    ...
    // Atualização final da velocidade com paralelismo
    #pragma omp parallel for
    for (i = 0; i < N; i++) {
        for (j = 0; j < 3; j++) {
            v[i][j] += 0.5 * a[i][j] * dt;
    ...
    // Cálculo da pressão com paralelismo e redução
