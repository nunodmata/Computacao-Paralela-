...
double Tinit = 100;  //voltar a mudar se quisermos passar como argumento
...
int main()
{
    ...
    if (n_threads < 1) {
        printf("Invalid number of threads. Using 1 thread.\n");
        n_threads = 1;
    }
    ...
    double rho = 35000; // voltar a mudar se quisermos passar como argumento
...
#pragma omp parallel for private(j, r2, rnorm, term1, term2)
	for (i = 0; i < N; i++) {
	    precalculated[i] = 0.0;
...
double VelocityVerlet(double dt, int iter, FILE *fp) {
    ...
    for (i=0; i<N; i++) {
            ...
            r[i][0] += v[i][1]*dt + 0.5*a[i][1]*dt*dt;
            r[i][0] += v[i][2]*dt + 0.5*a[i][2]*dt*dt;
            v[i][2] += 0.5*a[i][0]*dt;
            v[i][1] += 0.5*a[i][1]*dt;
            v[i][0] += 0.5*a[i][2]*dt;
